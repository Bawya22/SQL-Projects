...
-- Table to store information about stations
	...
	VALUES
		...
		(3, 'Neelesh Misra', 2, '2010-09-01'),
		(4, 'Bhawana Somaaya', 1, '2011-09-01'),
	...
	VALUES
        ...
        select shows.* from shows join hosts on shows.HostID = hosts.HostID where HostName="Vrajesh Hirjee";
        select shows.ShowID,shows.ShowName,count(awards.AwardName) as "Total Awards" from shows join awards on shows.ShowID=awards.ShowID group by ShowID;
        select showpartnerships.ShowID,shows.ShowName,partnerships.PartnerName from showpartnerships join shows join partnerships  on showpartnerships.ShowID =shows.ShowID and showpartnerships.PartnershipID = partnerships.PartnershipID where PartnerName ="Spotify";
        select * from hosts where JoinDate >2010;
        select ShowName,LaunchDate from shows order by LaunchDate desc;
        select hosts.HostID,hosts.HostName,hosts.ShowCount,count(ShowName) as "Total Shows" from hosts join shows on shows.HostID=hosts.HostID group by ShowID;
        select HostID,HostName,ShowCount from hosts;
        select PlatformName,Link from onlinepresence;
        select * from stations where Location="South" having LaunchDate >2007;
        select *,row_number() over( order by ShowCount desc) as "Rank" from hosts ;