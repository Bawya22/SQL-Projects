...
select destination.DestinationID,touristarrivals.Year,touristarrivals.NumberofVisitors from destination join touristarrivals on destination.DestinationID=touristarrivals.DestinationID where Name="Maldives" and Year=2019;
select destination.Name,destination.AverageBudgetPerDay,destination.BestTimeToVisit,touristarrivals.NumberofVisitors,touristarrivals.Year from destination join touristarrivals on destination.DestinationID=touristarrivals.DestinationID where NumberOfVisitors>10000 and Year=2022;
select Name,UniqueFeatures from destination where Name="Lakshadweep";
select destination.Name,indiantourists.* from destination join indiantourists on destination.DestinationID=indiantourists.DestinationID where Name="Maldives" and Year=2021;
